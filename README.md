...
1. Деплойте содержимое этого репозитория напрямую в каталог `/wp-content/plugins/gsp-children-portal/`.
2. Убедитесь, что главный файл доступен по пути `/wp-content/plugins/gsp-children-portal/gsp-children-portal.php`. Внутри каталога плагина не должно быть вложенной папки `gsp-children-portal/`.
...
Опциональные параметры ссылок для кнопок и служебной навигации:

```text
[gsp_children_portal application_url="/zayavka/" account_url="/lk/" programs_url="/programmy/" events_url="/meropriyatiya/"]
```
...
- Если featured image отсутствует, выводятся SVG-заглушки из `assets/img/`. Реальные иллюстрации для hero, программ, Skysmart, мероприятий и историй можно класть в этот каталог либо назначать как featured image записей.
...
1. Каталог в WordPress называется `/wp-content/plugins/gsp-children-portal/`.
2. Главный файл `gsp-children-portal.php` лежит прямо в этом каталоге, то есть в корне репозитория.
3. Файл лежит напрямую в папке плагина: `/wp-content/plugins/gsp-children-portal/gsp-children-portal.php`, а не в `/wp-content/plugins/gsp-children-portal/gsp-children-portal/gsp-children-portal.php`.
...
## Структура для прямого деплоя из Git

Репозиторий уже подготовлен так, чтобы его корень совпадал с корнем каталога плагина WordPress:

```text
repo-root/
├── gsp-children-portal.php
├── assets/
├── includes/
├── templates/
├── README.md
...
При деплое из Git указывайте целевой каталог `/wp-content/plugins/gsp-children-portal/`. Не создавайте дополнительную вложенную папку `gsp-children-portal` внутри этого каталога, иначе WordPress не найдёт заголовок плагина.
